...
#' @param plotcols A character vector of 5 colors to be used for the heatmaps of the 5 footprint patterns ("TF", "open", "upNuc", "Nuc", "downNuc").
#'
...
for (i in seq_along(patterns)){
  ...
  ht_list <- ht_list + Heatmap(matrix = as.matrix(footprint_percentages[,grep(paste0("^",patterns[i]), colnames(footprint_percentages))]),
                               border = TRUE,
