...
```{r load data}
#data(NomeMatrix, package = "dinoR")

NomeMatrix <- readRDS(system.file("extdata", "NomeMatrix.rds", package = "dinoR"))
NomeMatrix
```
...
avePlotData <- metaPlots(NomeMatrix=NomeMatrix,ROIgroup=matrix(
  unlist(strsplit(NomeMatrix$names,"_")),ncol=4,byrow=TRUE)[,1])
...
ROIgroup <-  matrix(unlist(strsplit(rowData(footprint_counts)$ROI,"_")),ncol=4,byrow=TRUE)[,1]
